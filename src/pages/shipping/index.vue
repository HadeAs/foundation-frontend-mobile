...
const { shipped, dismissLogistics, object, materials, detail, busy, disabled, canSubmit, notice, notify, clear, scan, openMaterials, closeMaterials, submit } = useShipping();
const input = ref('');
let active = false;
...
function continueLogistics() {
  const receipt = shipped.value;
  if (!receipt || busy.value) return;
  busy.value = true;
  uni.navigateTo({
    url: `/pages/logistics-create/index?scanCode=${encodeURIComponent(receipt.object.containerCode)}&shippingNo=${encodeURIComponent(receipt.shippingNo)}`,
    success: () => { dismissLogistics(); },
    fail: () => { notify('无法打开物流任务页面，请重试；成品发货已完成', true); },
...
<template>
  <view class="app-page application-page">
    <view class="app-nav">
      ...
      <view class="app-nav__title">成品发货</view><view />
    </view>
    <OperationNotice :text="notice?.text" :error="notice?.error" />
    <view class="scan-top">
      <ScanInput v-model="input" placeholder="扫描或输入位置/料车/料箱编码" :disabled="disabled" show-scan-button compact inline-errors
        @scan="handleScan" @error="notify($event, true)" />
      <view v-if="object" class="card object-card">
        <view class="card-heading"><text>对象信息</text><text class="tag">{{ object.object.containerType === 'VEHICLE' ? '载具' : '料箱' }}</text></view>
        <view class="info"><text>扫描编码</text><text>{{ object.scanCode }}</text></view>
        <view class="info"><text>容器编号</text><text>{{ object.object.containerCode }}</text></view>
        <view class="info"><text>当前位置</text><text>{{ object.object.location ? object.object.location.locationCode + ' · ' + object.object.location.locationName : '—' }}</text></view>
    ...
    <view class="content">
      <view v-if="!object" class="card empty">请先扫描作业对象<view class="empty-hint">识别后查看关联信息，再确认提交</view></view>
      <view v-else-if="!object.totalMaterialQuantity" class="card empty">当前对象无可处理物料<view class="empty-hint">请核对对象或扫描其他容器</view></view>
      <view v-else class="card list-card">
        <view class="card-heading"><text>{{ object.object.containerType === 'VEHICLE' ? '关联料箱' : '关联物料' }}</text><text>{{ object.object.containerType === 'VEHICLE' ? object.boxes.length + ' 箱' : object.totalMaterialQuantity + ' 件' }}</text></view>
    ...
    <view class="footer">
      ...
      <button class="primary" :disabled="!canSubmit" @tap="submit">成品发货</button>
    </view>
    <view v-if="shipped" class="modal-mask">
      <view class="modal" role="dialog" aria-modal="true" aria-label="是否创建物流任务">
        <view class="modal-title">是否创建物流任务</view>
        <view class="modal-content">成品发货已完成。是否为料车 {{ shipped.object.containerCode }} 创建物流任务？</view>
        <view class="modal-actions"><button :disabled="busy" @tap="dismissLogistics">否</button><button class="primary" :disabled="busy" @tap="continueLogistics">是</button></view>
      </view>
    </view>
