...
const { object, materials, detail, busy, disabled, canSubmit, notice, notify, clear, scan, openMaterials, closeMaterials, submit } = useWarehouseApplication();
const input = ref('');
let active = false;
...
<template>
  <view class="app-page application-page">
    <view class="app-nav">
      ...
      <view class="app-nav__title">缴库申请</view><view />
    </view>
    <OperationNotice :text="notice?.text" :error="notice?.error" />
    <view class="scan-top">
      <ScanInput v-model="input" placeholder="扫描或输入载具/箱子编码" :disabled="disabled" show-scan-button compact inline-errors
    ...
    <view class="footer">
      ...
      <button class="primary" :disabled="!canSubmit" @tap="submit">确认申请</button>
    </view>
