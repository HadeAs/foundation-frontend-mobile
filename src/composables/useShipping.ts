...
import { shippingApi as api, shippingRequest, type ShippingObject, type ShippingReceipt } from '../services/shipping';
...
export function useShipping() {
  const object = shallowRef<ShippingObject | null>(null);
  const detail = shallowRef<BoxMaterials | null>(null);
  const shipped = shallowRef<ShippingReceipt | null>(null);
  const busy = ref(false);
  const notice = ref<{ text: string; error: boolean } | null>(null);
  const disabled = computed(() => busy.value || !!detail.value || !!shipped.value);
  ...
  function clear() { generation++; object.value = null; detail.value = null; shipped.value = null; busy.value = false; notice.value = null; }
  ...
  async function scan(raw: string) {
    ...
    await run(async (current) => {
      ...
      if (!current()) return;
      if (!result || !validContainer(result.object) || !['BOX', 'VEHICLE', 'LOCATION'].includes(result.scanType) || result.scanCode !== code ||
        (result.scanType !== 'LOCATION' && (result.scanType !== result.object.containerType || result.object.containerCode !== code)) ||
        (result.scanType === 'LOCATION' && result.object.location && result.object.location.locationCode !== code) || !Array.isArray(result.boxes) || !integer(result.totalMaterialQuantity, 0) ||
  ...
  async function submit() {
    ...
    const input = shippingRequest(target);
    await run(async (current) => {
      const result = await api.submit(input);
      if (!current()) return;
      if (!result || result.requestId !== input.requestId || !text(result.shippingNo) || !validContainer(result.object) ||
        result.object.containerType !== target.object.containerType || result.object.containerCode !== target.object.containerCode ||
        typeof result.canCreateLogisticsTask !== 'boolean' || !Number.isFinite(Date.parse(result.processedTime))) {
        throw new Error('发货结果不完整，未能确认处理成功，请核实后重试');
      }
      clear();
      notify(`成品发货成功，发货单号：${result.shippingNo}`);
      if (result.object.containerType === 'VEHICLE' && result.canCreateLogisticsTask) shipped.value = result;
    });
  }
  function dismissLogistics() { shipped.value = null; }
  return { shipped, dismissLogistics, object, materials, detail, busy, disabled, canSubmit, notice, notify, clear, scan, openMaterials, closeMaterials, submit };
}
